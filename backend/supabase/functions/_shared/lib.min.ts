...
const VERTEX_LOCATION = "asia-northeast3";
...
async function getAccessToken(): Promise<string> {
  const token = Deno.env.get("VERTEX_ACCESS_TOKEN");
  if (!token) throw new Error("VERTEX_ACCESS_TOKEN missing");
  return token;
}
function modelEndpoint(model: GeminiModel): string {
  const m = model === "gemini-3-flash"
    ? "gemini-3-flash"
    : "gemini-3.1-flash-lite";
  return `https://${VERTEX_LOCATION}-aiplatform.googleapis.com/v1/projects/${PROJECT_ID}/locations/${VERTEX_LOCATION}/publishers/google/models/${m}:generateContent`;
}
export async function callGemini(req: GeminiRequest): Promise<GeminiResponse> {
  ...
  const resp = await fetch(modelEndpoint(req.model), {
    method: "POST",
    headers: {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`vertex_${resp.status}: ${text}`);
  }
